...
static int  s_idx_home[CS_MAX_MATCHES];   // 首页:法尔孔 live + upcoming
static int  s_n_home;
static int  s_idx_hist[CS_MAX_MATCHES];   // 战绩:法尔孔已结束
static int  s_n_hist;
static bool s_has_live;                    // 首页里有进行中的比赛(触发实时刷新)
...
static void refresh_indices(void)
{
    ...
    // 战绩:只看法尔孔已结束,没有就是空,绝不回退全局
    s_n_hist = collect(CS_ST_FINISHED, ft, s_idx_hist, CS_MAX_MATCHES, false);

    // 首页:法尔孔 live 在前、upcoming 在后;两者都只看法尔孔,不回退全局。
    // 这样首页既给实时比分(进行中)又给未来预告,战绩页给近期记录。
    s_n_home = 0;
    s_has_live = false;
    int n = collect(CS_ST_LIVE, ft, s_idx_home, CS_MAX_MATCHES, false);
    for (int i = 0; i < n; i++) s_has_live = true;
    int base = s_n_home; s_n_home = base + n;
    n = collect(CS_ST_UPCOMING, ft, s_idx_home + s_n_home, CS_MAX_MATCHES - s_n_home, false);
    s_n_home += n;

    int cap = (s_view == V_HIST) ? s_n_hist : s_n_home;
...
static void render_status(void)
{
    ...
    int cnt = (s_view == V_HIST) ? s_n_hist : s_n_home;
    if (s_view == V_HIST) {
        ...
        snprintf(buf, sizeof(buf), "赛程%d/%d", s_sel + 1, cnt);
    } else {
...
static void build_match(lv_obj_t *p)
{
    ...
    const cs_match_t *m = &cs_data()->m[s_idx_home[s_sel % s_n_home]];
...
static void build_portal(lv_obj_t *p)
{
    ...
    if (s_portal_page == 0) {
        panel(p, 4, 2, SCR_W - 8, 22, s_portal_on ? 0x152A1C : 0x3A2A14,
              s_portal_on ? C_GREEN : C_AMBER, 6);
        lv_obj_t *hd = label(p, 10, 5, s_portal_on ? "配网门户已开启" : "配网门户未开启",
                             &font_cn16, s_portal_on ? C_GREEN : C_AMBER);
        ...
        lv_obj_t *t3 = label(p, 12, 252,
                             s_portal_on ? "停止配网:OK 键回设置页"
                                         : "未开启:设置页点『打开配网门户』",
                             &font_cn16, s_portal_on ? C_MUTED : C_AMBER);
...
static void exec_set(void)
{
    ...
    case 3:
        // 配网信息/二维码:无论门户是否已开都进入信息页(门户未开时给出提示)
        s_back = V_SET;
        s_portal_page = 0;
        s_view = V_PORTAL;
        break;
...
static void move_sel(int dir)
{
    switch (s_view) {
    case V_MATCH:
        if (s_n_home > 1) { s_sel = (s_sel + dir + s_n_home) % s_n_home; s_dirty = true; }
        break;
    case V_HIST:
