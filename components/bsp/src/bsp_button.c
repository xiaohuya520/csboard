...
static TickType_t s_last_press[BSP_BTN_COUNT];
...
static void on_event(void *arg, void *usr_data, bsp_btn_ev_t ev) {
    ...
    // (用 FreeRTOS tick 而不是 esp_timer:bsp 组件不必再多一个组件依赖。)
    if (ev == BSP_BTN_PRESS) {
        TickType_t now = xTaskGetTickCount();
        if (s_last_press[idx] && (now - s_last_press[idx]) < pdMS_TO_TICKS(80)) return;
        s_last_press[idx] = now;
    }
